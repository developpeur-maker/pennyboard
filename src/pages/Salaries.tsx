...
const Salaries: React.FC = () => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case 'joursTravailles':
          aValue = a.joursTravailles ?? 0
          bValue = b.joursTravailles ?? 0
          break
        default:
          return 0
      }
  ...
  const filteredTotals = useMemo(() => {
    ...
    return filteredAndSortedEmployees.reduce((acc, employee) => ({
      ...
      totalJoursTravailles: acc.totalJoursTravailles + (employee.joursTravailles ?? 0)
    }), {
      totalSalaryPaid: 0,
      totalPrimes: 0,
      totalContributions: 0,
            ...
            <table className="w-full">
                ...
                {filteredAndSortedEmployees.map((employee, index) => {
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900">
                      {employee.joursTravailles != null ? Number(employee.joursTravailles).toFixed(2) : '—'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900">
              ...
              <tfoot className="bg-gray-50">
                <tr>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold text-gray-900">
                    {filteredTotals.totalJoursTravailles != null ? filteredTotals.totalJoursTravailles.toFixed(2) : '—'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold text-gray-900">
                    {formatCurrency(totalSalaryPaid)}
                  </td>
